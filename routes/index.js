...
////////////////////////////////////////////////////////////////////////////////
/* GET LIST OF DISTINCT DEPARTMENTS FROM MONGODB. */
  ...
  if(!err) {
    ...
    var collection = db.collection('mydb');
     //collection.find().toArray(function(err, items) {});
      ...
      collection.find({type:'author'}).toArray(function(err, result) {
...
router.get('/dept/:department', function(req, res, next) {
  ...
  if(!err) {
    ...
    collection.find({author: req.params.department}).toArray(function (err, result) {
      if (err) {
        ...
        console.log('Found:', result);
         res.render('displayDepartments', { title: req.params.department , result: JSON.stringify(result)});
